defmodule IslandsEngine.Player do
  ...
  @doc"""
  Before setting coordinates on an island, the list of coordinates is updated
  """
  ...
  #Loop through all coordinates and return a list of coordinate agents
  defp convert_coordinates(board, coordinates) do
    Enum.map(coordinates, fn coord -> convert_coordinate(board, coord) end)
  end

  #If the coordinate is an atom use the Board function to retun the coordinate agent
  ...
  @doc"""
  Winning means foresting all your opponent's islands
  """
  def win?(opponent) do
    opponent
